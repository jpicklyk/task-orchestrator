...
- **A loopback publish alone does not stop DNS rebinding** — a page loaded from an attacker-controlled hostname that resolves to `127.0.0.1` (or is rebound to it after load) can make the victim's own browser send a same-origin request straight through the loopback publish. A `Host`-header allowlist closes that gap independently of the publish mapping: every HTTP request (`/mcp` and every `/api/v1` route) is rejected with `403 host_not_allowed` unless its `Host` is `localhost`/`127.0.0.1`/`[::1]` (any port, the always-on default) or listed in `MCP_ALLOWED_HOSTS`. This is enforced by the server regardless of `API_AUTH_MODE`; see [`current/docs/fleet-deployment.md`](current/docs/fleet-deployment.md).
