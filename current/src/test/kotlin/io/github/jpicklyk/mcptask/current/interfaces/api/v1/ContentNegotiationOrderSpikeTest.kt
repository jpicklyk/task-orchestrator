...
class ContentNegotiationOrderSpikeTest {
    ...
    @Serializable
    data class PingResponse(val message: String, val value: Int)
     ...
     */
    @Test
    fun `ContentNegotiation with McpJson installed first serves REST JSON correctly`() = testApplication {
        application {
     ...
     */
    @Test
    fun `McpJson explicitNulls=false does not break REST JSON serialization`() = testApplication {
        application {
            install(ContentNegotiation) {
                json(McpJson)
            }
     ...
     */
    @Test
    fun `serviceRoutes info endpoint serializes correctly under McpJson ContentNegotiation`() = testApplication {
        application {
            install(ContentNegotiation) {
                json(McpJson)
            }
     ...
     */
    @Test
    fun `wellKnownRoutes returns correct JSON under McpJson ContentNegotiation`() = testApplication {
        application {
            install(ContentNegotiation) {
                json(McpJson)
            }
     ...
     */
    @Test
    fun `CORS and ContentNegotiation install without conflict`() = testApplication {
        application {
            install(ContentNegotiation) {
                json(McpJson)
            }
