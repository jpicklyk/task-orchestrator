...
class DefaultMcpLoggingServiceTest {
    ...
    @Test
    fun `log does not throw when no server is bound`(): Unit =
        runBlocking {
    ...
    @Test
    fun `convenience methods do not throw when no server is bound`(): Unit =
        runBlocking {
    ...
    @Test
    fun `log is a no-op when sessions map is empty`(): Unit =
        runBlocking {
    ...
    @Test
    fun `log sends notification with correct LoggingLevel to each session`(): Unit =
        runBlocking {
    ...
    @Test
    fun `exceptions from sendLoggingMessage are swallowed`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns mapOf("session-1" to session)
    ...
    @Test
    fun `exception in one session does not prevent delivery to other sessions`(): Unit =
        runBlocking {
            val session1 = mockk<ServerSession>(relaxed = true)
            val session2 = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns
    ...
    @Test
    fun `debug convenience method sends LoggingLevel Debug`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
    ...
    @Test
    fun `info convenience method sends LoggingLevel Info`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns mapOf("s" to session)
            coEvery { server.sendLoggingMessage(any(), any()) } returns Unit
    ...
    @Test
    fun `warning convenience method sends LoggingLevel Warning`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns mapOf("s" to session)
            coEvery { server.sendLoggingMessage(any(), any()) } returns Unit
    ...
    @Test
    fun `error convenience method sends LoggingLevel Error`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns mapOf("s" to session)
            coEvery { server.sendLoggingMessage(any(), any()) } returns Unit
    ...
    @Test
    fun `toSdkLevel maps all McpLogLevel values correctly`() {
    ...
    @Test
    fun `log with data map includes data in notification payload`(): Unit =
        runBlocking {
            val session = mockk<ServerSession>(relaxed = true)
            every { server.sessions } returns mapOf("s" to session)
            coEvery { server.sendLoggingMessage(any(), any()) } returns Unit
