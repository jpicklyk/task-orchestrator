...
class ItemCreateRouteTest {
    ...
    fun `POST items without WRITE_ITEMS capability returns 403`() : Unit =
        testApplication {
            ...
            assertTrue((items as Result.Success).data.none { it.title == "Should Not Create" },
...
class ItemPatchRouteTest {
    ...
    fun `PATCH items id updates title and returns 200 with new ETag`() : Unit =
        testApplication {
            ...
            val response = client.patch("/api/v1/items/${item.id}") {
                header("Authorization", "Bearer $WRITE_TOKEN")
    ...
    fun `PATCH items null field in patch body removes field value`() : Unit =
        testApplication {
            ...
            val persisted = runBlocking { repo.workItemRepository().getById(item.id) }
            assertFalse((persisted as Result.Success).data.description != null &&
    ...
    fun `PATCH items absent field in patch body leaves field unchanged`() : Unit =
        testApplication {
            ...
            assertEquals("keep me", (persisted as Result.Success).data.description,
    ...
    fun `PATCH items parentId null moves item to root`() : Unit =
        testApplication {
            ...
            val response = client.patch("/api/v1/items/${child.id}") {
                header("Authorization", "Bearer $WRITE_TOKEN")
                header(HttpHeaders.IfMatch, etag)
                contentType(ContentType.Application.Json)
    ...
    fun `PATCH items post-merge domain validation failure returns 400`() : Unit =
        testApplication {
            ...
            assertEquals(HttpStatusCode.BadRequest, response.status)
            val body = response.bodyAsText()
            assertTrue(body.contains("validation_error") || body.contains("complexity"),
                "Should report validation error for out-of-range complexity: $body")

            // Hard assertion: complexity unchanged in DB
            val persisted = runBlocking { repo.workItemRepository().getById(item.id) }
            assertNull((persisted as Result.Success).data.complexity,
...
class ItemDeleteRouteTest {
    ...
    fun `DELETE items id deletes and returns 204`() : Unit =
        testApplication {
            ...
            val response = client.delete("/api/v1/items/${item.id}") {
                header("Authorization", "Bearer $WRITE_TOKEN")
            }
...
class AdvanceRouteTest {
    ...
    fun `POST items id advance starts item and returns 200 with new role`() : Unit =
        testApplication {
            ...
            val response = client.post("/api/v1/items/${item.id}/advance") {
                header("Authorization", "Bearer $WRITE_TOKEN")
                contentType(ContentType.Application.Json)
            ...
            val persisted = runBlocking { repo.workItemRepository().getById(item.id) }
            assertTrue((persisted as Result.Success).data.role.name.lowercase() != "queue",
    ...
    fun `POST items id advance creates role_transitions row with API actor`() : Unit =
        testApplication {
            ...
            assertEquals("api:$WRITE_TOKEN_ID", transition.actorClaim?.id,
                "Actor id should be 'api:<tokenId>'")
            assertEquals("external", transition.actorClaim?.kind?.toJsonString(),
    ...
    fun `POST items id advance without ADVANCE capability returns 403`() : Unit =
        testApplication {
            ...
            val persisted = runBlocking { repo.workItemRepository().getById(item.id) }
            assertEquals("queue", (persisted as Result.Success).data.role.name.lowercase())
        }
    ...
    fun `POST items id advance response body does not contain claimedBy`() : Unit =
        testApplication {
            ...
            if (response.status == HttpStatusCode.OK) {
                val body = response.bodyAsText()
                assertFalse(body.contains("claimedBy"),
...
class NoteWriteRouteTest {
    ...
    fun `PUT note creates new note and persists with api actor`() : Unit =
        testApplication {
            ...
            val response = client.put("/api/v1/items/${item.id}/notes/impl-note") {
                header("Authorization", "Bearer $WRITE_TOKEN")
                contentType(ContentType.Application.Json)
            ...
            assertEquals("api:$WRITE_TOKEN_ID", note.actorClaim?.id,
                "Actor id must be 'api:<tokenId>'")
            assertEquals("external", note.actorClaim?.kind?.toJsonString(),
    ...
    fun `PUT note updates existing note`() : Unit =
        testApplication {
            ...
            val readResponse = client.get("/api/v1/items/${item.id}/notes/update-me") {
                header("Authorization", "Bearer $WRITE_TOKEN")
            }
            ...
            val response = client.put("/api/v1/items/${item.id}/notes/update-me") {
                header("Authorization", "Bearer $WRITE_TOKEN")
    ...
    fun `PUT note with stale ETag on update returns 412`() : Unit =
        testApplication {
            ...
            val response = client.put("/api/v1/items/${item.id}/notes/stale-test") {
                header("Authorization", "Bearer $WRITE_TOKEN")
                header(HttpHeaders.IfMatch, "\"v1-0\"") // stale
                contentType(ContentType.Application.Json)
    ...
    fun `PUT note without WRITE_NOTES capability returns 403`() : Unit =
        testApplication {
            ...
            val response = client.put("/api/v1/items/${item.id}/notes/new-note") {
                header("Authorization", "Bearer $READ_ONLY_TOKEN")
                contentType(ContentType.Application.Json)
    ...
    fun `DELETE note deletes it and returns 204`() : Unit =
        testApplication {
            ...
            val response = client.delete("/api/v1/items/${item.id}/notes/delete-me") {
                header("Authorization", "Bearer $WRITE_TOKEN")
            }
            assertEquals(HttpStatusCode.NoContent, response.status)
...
class DependencyWriteRouteTest {
    ...
    fun `POST dependencies creates edge and returns 201`() : Unit =
        testApplication {
            ...
            val response = client.post("/api/v1/dependencies") {
                header("Authorization", "Bearer $WRITE_TOKEN")
                contentType(ContentType.Application.Json)
            ...
            val deps = runBlocking { kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
    ...
    fun `DELETE dependencies id deletes edge and returns 204`() : Unit =
        testApplication {
            ...
            val response = client.delete("/api/v1/dependencies/${dep.id}") {
                header("Authorization", "Bearer $WRITE_TOKEN")
            }
            assertEquals(HttpStatusCode.NoContent, response.status)

            // Hard assertion: dep gone
            val remaining = runBlocking { kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
