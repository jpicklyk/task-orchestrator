...
class ManageItemsToolTest {
    ...
    private fun contextWithSchema(service: NoteSchemaService) = ToolExecutionContext(repositoryProvider, service)
    ...
    @Test
    fun `create single item with title only`() =
        runBlocking {
            val result =
                tool.execute(
                    params(
    ...
    @Test
    fun `create item with all fields`() =
        runBlocking {
            val result =
                tool.execute(
                    params(
    ...
    @Test
    fun `create item with parentId computes depth`() =
        runBlocking {
            // Create parent
            val parentResult =
                tool.execute(
            ...
            val parentId =
                (parentResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
    ...
    fun `create item rejects depth beyond MAX_DEPTH`() =
        runBlocking {
            ...
            val depth3Id =
                (depth3Result["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
    ...
    @Test
    fun `create item with invalid parentId fails`() =
        runBlocking {
    ...
    @Test
    fun `create batch with mixed success and failure`() =
        runBlocking {
            val nonexistentId = UUID.randomUUID().toString()
    ...
    @Test
    fun `create item with shared parentId default`() =
        runBlocking {
            // Create parent
            val parentResult =
                tool.execute(
    ...
    @Test
    fun `create item with invalid tags fails`() =
        runBlocking {
            val result =
                tool.execute(
                    params(
    ...
    @Test
    fun `create item with tags matching schema includes tags and expectedNotes`(): Unit =
        runBlocking {
            val mockSchema =
                object : NoteSchemaService {
                    override fun getSchemaForTags(tags: List<String>): List<NoteSchemaEntry>? =
                        if (tags.contains("feature-plan")) {
                            listOf(
                                NoteSchemaEntry(
                                    key = "requirements",
                                    role = "queue",
    ...
    @Test
    fun `create item with no tags includes tags null and no expectedNotes`(): Unit =
        runBlocking {
            val result =
                tool.execute(
                    params(
    ...
    @Test
    fun `create item with tags not matching any schema includes tags but no expectedNotes`(): Unit =
        runBlocking {
            val mockSchema =
                object : NoteSchemaService {
    ...
    @Test
    fun `create with self-parent parentId is rejected`(): Unit =
        runBlocking {
    ...
    @Test
    fun `create with parent that would form cycle is rejected`(): Unit =
        runBlocking {
            // Create item A (root)
            val aResult =
                tool.execute(
            ...
            val aId =
                (aResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
                                ...
                                listOf(
                                    buildJsonObject {
                                        ...
                                        put(
                                            "parentId",
                                            JsonPrimitive(
                                                (bResult["data"] as JsonObject)["items"]!!
                                                    .jsonArray[0]
                                                    .jsonObject["id"]!!
                                                    .jsonPrimitive.content
                                            )
    ...
    @Test
    fun `update partial fields preserves existing`() =
        runBlocking {
    ...
    @Test
    fun `update parentId recomputes depth`() =
        runBlocking {
            ...
            val rootAId =
                (rootAResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
            ...
            val rootBId =
                (rootBResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
            ...
            val childId =
                (childResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
    ...
    @Test
    fun `update item with self as parent is rejected`(): Unit =
        runBlocking {
            // Create an item
            val createResult =
                tool.execute(
    ...
    @Test
    fun `update item to descendant as parent is rejected`(): Unit =
        runBlocking {
            // Create root item A
            val rootResult =
                tool.execute(
            ...
            val rootId =
                (rootResult["data"] as JsonObject)["items"]!!
                    .jsonArray[0]
                    .jsonObject["id"]!!
                    .jsonPrimitive.content
    ...
    @Test
    fun `update nonexistent item fails`() =
        runBlocking {
    ...
    @Test
    fun `delete nonexistent item returns failure`() =
        runBlocking {
            val randomId = UUID.randomUUID().toString()
            val result =
                tool.execute(
    ...
    @Test
    fun `recursive delete removes parent and all descendants`(): Unit =
        runBlocking {
            // Create root (depth 0)
            val rootResult =
                tool.execute(
    ...
    @Test
    fun `non-recursive delete of parent with children fails gracefully`(): Unit =
        runBlocking {
    ...
    @Test
    fun `recursive delete of leaf item with no children succeeds`(): Unit =
        runBlocking {
    ...
    @Test
    fun `create item with null guidance omits guidance field from expectedNotes`(): Unit =
        runBlocking {
            val mockSchema =
                object : NoteSchemaService {
                    override fun getSchemaForTags(tags: List<String>): List<NoteSchemaEntry>? =
    ...
    @Test
    fun `update with role field is rejected`(): Unit =
        runBlocking {
    ...
    @Test
    fun `update without role field succeeds`(): Unit =
        runBlocking {
            // Create an item first
            val createResult =
                tool.execute(
