...
class GetContextToolTest {
    ...
    @Test
    fun `item mode with no schema (NoOp) returns empty schema and canAdvance=true`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item mode with schema and missing required note returns canAdvance=false`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
    ...
    @Test
    fun `item mode with all required notes filled returns canAdvance=true`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
            val item = makeItem(id = itemId, role = Role.WORK, tags = "feature-task")
    ...
    @Test
    fun `session resume mode with valid ISO timestamp returns mode=session-resume`(): Unit =
        runBlocking {
    ...
    @Test
    fun `invalid itemId UUID returns error response`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
    ...
    @Test
    fun `item mode returns item fields correctly`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
    ...
    @Test
    fun `session resume includes active items from work and review roles`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item mode includeAncestors=true adds ancestors array to item`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item mode includeAncestors=false does not add ancestors array`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
    ...
    @Test
    fun `item mode root item includeAncestors=true has empty ancestors array`(): Unit =
        runBlocking {
            val rootId = UUID.randomUUID()
    ...
    @Test
    fun `health check includeAncestors=true adds ancestors to active and blocked items`(): Unit =
        runBlocking {
            val rootId = UUID.randomUUID()
            val childId = UUID.randomUUID()
    ...
    @Test
    fun `session resume with limit=10 passes 10 to findSince`(): Unit =
        runBlocking {
            val since = Instant.now().minusSeconds(3600)
            val capturedLimit = slot<Int>()
    ...
    @Test
    fun `session resume with limit clamped to max 200`(): Unit =
        runBlocking {
            val since = Instant.now().minusSeconds(3600)
            val capturedLimit = slot<Int>()
    ...
    @Test
    fun `session resume with limit clamped to min 1`(): Unit =
        runBlocking {
            val since = Instant.now().minusSeconds(3600)
            val capturedLimit = slot<Int>()
    ...
    @Test
    fun `session resume includeAncestors=true adds ancestors to active items`(): Unit =
        runBlocking {
            val since = Instant.now().minusSeconds(3600)
            val rootId = UUID.randomUUID()
            val childId = UUID.randomUUID()
    ...
    @Test
    fun `health check parallel queries return correct combined results`(): Unit =
        runBlocking {
    ...
    @Test
    fun `canAdvance is false for terminal item even with no schema`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
    ...
    @Test
    fun `canAdvance is false for terminal item with schema`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
            ...
            val gateStatus = data["gateStatus"]!!.jsonObject
            assertFalse(
                gateStatus["canAdvance"]!!.jsonPrimitive.boolean,
    ...
    @Test
    fun `health-check stalled item with multiple missing required notes lists all of them`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
            val item = makeItem(id = itemId, role = Role.WORK, tags = "feature-task")
    ...
    @Test
    fun `health-check stalled item includeAncestors=true includes ancestors in stalled item entry`(): Unit =
        runBlocking {
            val rootId = UUID.randomUUID()
            val childId = UUID.randomUUID()
    ...
    @Test
    fun `session-resume stalled item includeAncestors=true includes ancestors`(): Unit =
        runBlocking {
            val since = Instant.now().minusSeconds(3600)
            val rootId = UUID.randomUUID()
            val childId = UUID.randomUUID()
    ...
    @Test
    fun `item mode noteProgress is null when no schema matches`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
            val item = makeItem(id = itemId, role = Role.WORK, tags = null)
    ...
    @Test
    fun `item mode noteProgress is null for terminal items`(): Unit =
        runBlocking {
            val itemId = UUID.randomUUID()
            val item = makeItem(id = itemId, role = Role.TERMINAL, tags = "feature-task")
