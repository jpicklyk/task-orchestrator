...
class SQLiteWorkItemRepositoryAncestorCycleTest {
    ...
    @Test
    fun `findAncestorChains with self-loop returns empty chain`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findAncestorChains with mutual cycle terminates and returns at most one ancestor`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findAncestorChains with valid linear chain returns correct ancestors`(): Unit =
        runBlocking {
