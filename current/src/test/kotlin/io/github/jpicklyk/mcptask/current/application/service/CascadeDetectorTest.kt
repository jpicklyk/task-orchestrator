...
class CascadeDetectorTest {
    ...
    inner class DetectCascades {
        @Test
        fun `no cascade when parentId is null (root item)`() =
            runBlocking {
        ...
        @Test
        fun `no cascade when not all children are terminal`() =
            runBlocking {
        ...
        @Test
        fun `cascade fires when all children are terminal`() =
            runBlocking {
                val parentId = UUID.randomUUID()
        ...
        @Test
        fun `recursive cascade fires for grandparent`() =
            runBlocking {
        ...
        @Test
        fun `no cascade when countChildrenByRole returns error`() =
            runBlocking {
                val parentId = UUID.randomUUID()
                val child = workItem(parentId = parentId, role = Role.TERMINAL)
        ...
        @Test
        fun `no cascade when children map is empty`() =
            runBlocking {
                val parentId = UUID.randomUUID()
                val child = workItem(parentId = parentId, role = Role.TERMINAL)
    ...
    inner class DetectStartCascades {
        @Test
        fun `child in WORK with parent in QUEUE returns cascade event`() =
            runBlocking {
                val parentId = UUID.randomUUID()
        ...
        @Test
        fun `child in WORK with parent already in WORK returns empty`() =
            runBlocking {
                val parentId = UUID.randomUUID()
                val parent = workItem(id = parentId, role = Role.WORK)
                val child = workItem(parentId = parentId, role = Role.WORK)
        ...
        @Test
        fun `child not in WORK returns empty immediately`() =
            runBlocking {
                val parentId = UUID.randomUUID()
        ...
        @Test
        fun `child in WORK with no parent returns empty`() =
            runBlocking {
        ...
        @Test
        fun `child in WORK with parent in REVIEW returns empty`() =
            runBlocking {
                val parentId = UUID.randomUUID()
    ...
    inner class FindUnblockedItems {
        @Test
        fun `empty dependency list returns no unblocked items`() =
            runBlocking {
        ...
        @Test
        fun `item becomes unblocked when blocker reaches threshold`() =
            runBlocking {
        ...
        @Test
        fun `item stays blocked when one blocker has not reached threshold`() =
            runBlocking {
        ...
        @Test
        fun `RELATES_TO dependencies are ignored in unblock detection`() =
            runBlocking {
        ...
        @Test
        fun `item unblocks with custom unblockAt threshold`() =
            runBlocking {
                val blockerId = UUID.randomUUID()
                val targetId = UUID.randomUUID()
        ...
        @Test
        fun `item stays blocked when blocker has not reached custom threshold`() =
            runBlocking {
                val blockerId = UUID.randomUUID()
                val targetId = UUID.randomUUID()
        ...
        @Test
        fun `BLOCKED role never satisfies unblock threshold`() =
            runBlocking {
                val blockerId = UUID.randomUUID()
                val targetId = UUID.randomUUID()
        ...
        @Test
        fun `RELATES_TO incoming deps on target are ignored during unblock check`() =
            runBlocking {
                val blockerId = UUID.randomUUID()
