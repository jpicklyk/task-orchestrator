...
class WorkItemRepositoryGraphTest {
    ...
    @Test
    fun `findByIds with single existing id returns that item`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findByIds with multiple ids returns all matching items`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findByIds with nonexistent ids silently omits them`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findByIds with only nonexistent ids returns empty list`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findAncestorChains for depth-0 root item returns empty ancestor list`(): Unit =
        runBlocking {
    ...
    @Test
    fun `findAncestorChains for depth-1 item returns one ancestor (the root)`(): Unit =
        runBlocking {
            val root = createItem("Root", parentId = null, depth = 0)
    ...
    @Test
    fun `findAncestorChains for depth-2 item returns two ancestors root-first`(): Unit =
        runBlocking {
            val root = createItem("Root", parentId = null, depth = 0)
    ...
    @Test
    fun `findAncestorChains for batch of mixed-depth items resolves correctly`(): Unit =
        runBlocking {
            val root = createItem("Root", parentId = null, depth = 0)
            val child = createItem("Child", parentId = root.id, depth = 1)
