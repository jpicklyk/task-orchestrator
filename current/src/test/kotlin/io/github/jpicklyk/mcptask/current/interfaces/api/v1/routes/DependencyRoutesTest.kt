...
class DependencyRoutesTest {
    ...
    fun `GET items id dependencies returns empty buckets when no deps`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${item.id}/dependencies") {
    ...
    fun `GET items id dependencies blocks bucket populated when item blocks another`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${blocker.id}/dependencies") {
                    header("Authorization", "Bearer $TEST_TOKEN")
                }
            assertEquals(HttpStatusCode.OK, response.status)
            val body = response.bodyAsText()
    ...
    fun `GET items id dependencies blockedBy bucket populated`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${blocked.id}/dependencies") {
                    header("Authorization", "Bearer $TEST_TOKEN")
                }
            assertEquals(HttpStatusCode.OK, response.status)
            val body = response.bodyAsText()
    ...
    fun `GET items id dependencies related bucket populated`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${a.id}/dependencies") {
                    header("Authorization", "Bearer $TEST_TOKEN")
                }
            assertEquals(HttpStatusCode.OK, response.status)
            val body = response.bodyAsText()
    ...
    fun `GET items id backlinks returns referencing items`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${target.id}/backlinks") {
                    header("Authorization", "Bearer $TEST_TOKEN")
                }
            assertEquals(HttpStatusCode.OK, response.status)
            val body = response.bodyAsText()
    ...
    fun `GET items id dependencies returns 404 for missing item`() =
        testApplication {
            ...
            val response =
                client.get("/api/v1/items/${UUID.randomUUID()}/dependencies") {
                    header("Authorization", "Bearer $TEST_TOKEN")
                }
