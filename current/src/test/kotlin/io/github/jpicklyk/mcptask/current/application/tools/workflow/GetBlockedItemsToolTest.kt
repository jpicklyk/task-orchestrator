...
class GetBlockedItemsToolTest {
    ...
    @Test
    fun `no blocked items returns empty result`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item explicitly in BLOCKED role is included with blockType explicit`(): Unit =
        runBlocking {
    ...
    @Test
    fun `explicitly BLOCKED item with no deps has empty blockedBy array`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item in QUEUE with unsatisfied BLOCKS dep is included with blockType dependency`(): Unit =
        runBlocking {
    ...
    @Test
    fun `item in WORK with unsatisfied dep is included`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
    ...
    @Test
    fun `item in QUEUE with all deps satisfied is NOT included`(): Unit =
        runBlocking {
    ...
    @Test
    fun `unblockAt work with blocker at WORK means dep satisfied, item NOT included`(): Unit =
        runBlocking {
    ...
    @Test
    fun `unblockAt work with blocker at QUEUE means dep unsatisfied, item included`(): Unit =
        runBlocking {
    ...
    @Test
    fun `parentId scoping only returns items under that parent`(): Unit =
        runBlocking {
    ...
    @Test
    fun `items in TERMINAL role are never included`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
    ...
    @Test
    fun `RELATES_TO deps are ignored for blocking`(): Unit =
        runBlocking {
    ...
    @Test
    fun `multiple blockers all listed in blockedBy array with correct count`(): Unit =
        runBlocking {
            val blocker1 = createItem("Blocker 1", role = "queue")
            val blocker2 = createItem("Blocker 2", role = "queue")
    ...
    @Test
    fun `IS_BLOCKED_BY dep correctly identified`(): Unit =
        runBlocking {
    ...
    @Test
    fun `blocker entry includes effectiveUnblockRole defaulting to terminal`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
    ...
    @Test
    fun `item in REVIEW with unsatisfied dep is included`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
    ...
    @Test
    fun `userSummary returns correct message`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
            val blockedId = createItem("Blocked", role = "queue")
            createDep(blockerId, blockedId)
    ...
    @Test
    fun `includeAncestors=true adds ancestors array to each blocked item`(): Unit =
        runBlocking {
    ...
    @Test
    fun `includeAncestors=true root blocked item has empty ancestors array`(): Unit =
        runBlocking {
    ...
    @Test
    fun `includeAncestors=false default omits ancestors key`(): Unit =
        runBlocking {
            val blockerId = createItem("Blocker", role = "queue")
            val blockedId = createItem("Blocked", role = "queue")
            createDep(blockerId, blockedId)
