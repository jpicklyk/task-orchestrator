...
- `get_agent_definition`, `recommend_agent` - Agent automation
...
**New Tools (1):**
- **`setup_project`** - Core project setup (creates `.taskorchestrator/` configuration for all MCP clients)

**Changed Tools (1):**
- **`setup_claude_orchestration`** - Now Claude Code specific (creates `.claude/` directory, requires `setup_project` to run first)
...
#### Background: Separation of Core vs Claude-Specific Setup

In early v2.0, `setup_claude_orchestration` handled both core configuration (`.taskorchestrator/`) and Claude Code specific files (`.claude/`). This was problematic because:
- `.taskorchestrator/` configuration is needed by all MCP clients
- `.claude/` agent definitions are Claude Code specific
- Users of other MCP clients (Cursor, Windsurf, Claude Desktop) had to run a "Claude orchestration" tool for core setup

**v2.0 Solution**: Split setup into two tools with clear responsibilities.

#### New Tool: setup_project

**Purpose**: Core Task Orchestrator setup for ALL MCP clients.

**What it creates**:
- `.taskorchestrator/` directory
- `.taskorchestrator/config.yaml` - Status progression and validation rules
...
**Usage**:
```
Ask AI: "Run setup_project"
```

**Who needs this**: Everyone (Claude Desktop, Claude Code, Cursor, Windsurf, all MCP clients)

#### Changed Tool: setup_claude_orchestration

**Old behavior** (pre-split):
- Created `.taskorchestrator/` directory
- Created `.taskorchestrator/config.yaml`, `status-workflow-config.yaml`, `agent-mapping.yaml`
- Created `.claude/agents/` directory
- Created `.claude/skills/` directory
- Created `.claude/output-styles/` directory

**New behavior** (post-split):
- **Prerequisite check**: Requires `.taskorchestrator/` to exist first
- **Error if prerequisite not met**: "Run setup_project tool first to create core Task Orchestrator configuration"
- **Claude Code specific only**:
  - Creates `.claude/agents/task-orchestrator/` directory
  - Creates `.claude/skills/` directory
  - Creates `.claude/output-styles/` directory
  - Injects decision gates into CLAUDE.md

**Migration impact**: None for existing users (directories already exist). New users follow proper setup sequence.
...
Step 1: Run setup_project
  → Creates .taskorchestrator/ with core config
```

**For Claude Code users (optional)**:
```
Step 2: Run setup_claude_orchestration
  → Creates .claude/ with agents, skills, output-style
...
**Problem solved**:
- ✅ Clear separation: Core (all clients) vs Claude Code specific
- ✅ Better naming: `setup_project` vs `setup_claude_orchestration` is immediately clear
- ✅ No confusion: Users of Cursor/Windsurf don't need to run "Claude" tools
- ✅ Prerequisite validation: `setup_claude_orchestration` checks for core config

**Compatibility**:
- Existing installations: No changes needed (directories already exist)
- New installations: Follow proper setup sequence
- Tool behavior: Idempotent (safe to run multiple times)
