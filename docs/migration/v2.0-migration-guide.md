...
Task Orchestrator v2.0 introduces **container-based tool consolidation** that reduces the tool count from **56 tools to 16 tools** (71% reduction), achieving massive token savings and providing a consistent, permission-separated interface across all operations.
...
| `get_next_task` | `query_container` | `search` + filters | `task` |
| `get_blocked_tasks` | `query_container` | `search` + status filter | `task` |
...
- ✅ **71% tool reduction** (56 tools → 16 tools)
